...
import '../beach/beach-page.css'
import '../beach/beach-page-scale.css'
...
export function GuestHousePage({ onNavigate }) {
  ...
  const guesthouseOffers = useMemo(
    () => homeFeatured.filter((item) => item.category.split(' ').includes('guesthouse')),
    []
  )
  ...
  const visibleOffers = useMemo(() => {
    ...
    if (!filterCity) return guesthouseOffers
    return guesthouseOffers.filter((item) => normalize(item.location).includes(filterCity))
  }, [guesthouseOffers, city, cityQuery])
  ...
  return (
    <div className="beach-page guesthouse-page" data-testid="page-guesthouse">
      <section className="beach-hero" aria-label="Collection Maison d’hôte">
        <img className="beach-hero__image guesthouse-hero__image" src="/Movera-host1/assets/guesthouse-page-hero.jpeg" alt="Maison d’hôte Movera" decoding="async" fetchPriority="high" />\n        <div className="beach-hero__veil" />
        <div className="beach-hero__top">
          ...
          <span className="beach-hero__counter">{guesthouseOffers.length} séjours</span>
        </div>
        <div className="beach-hero__copy">
          <span>Collection Maison d’hôte</span>
          <h1>L’accueil tunisien,<br/>autrement.</h1>
          <p>Des maisons de caractère choisies pour leur charme et leur hospitalité.</p>
        </div>
      </section>
      ...
      <section className="beach-results" aria-live="polite">
        <header className="beach-results__head">
          <div>
            <span>{city || cityQuery ? `Séjours à ${city || cityQuery}` : 'Toutes les maisons d’hôte'}</span>
          ...
          {visibleOffers.map((item) => (
            <article className="beach-offer" key={item.id} onClick={() => onNavigate(`/listing/${item.id}`)}>
              <div className="beach-offer__media">
                ...
                <span className="beach-offer__badge">{item.badge || 'Maison d’hôte'}</span>
        ...
        {!visibleOffers.length ? (
          <div className="beach-empty">
            <span>◌</span>
            <h3>Pas encore de maison d’hôte ici.</h3>
