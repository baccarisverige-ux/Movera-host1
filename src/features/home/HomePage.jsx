...
function ListingCard({item,sectionId,favorite,toggleFavorite}){return <article className="b225-offer-card" data-testid={`home-card-${sectionId}-${item.id}`} aria-label={`${item.title}, ${item.priceTotal}, note ${item.rating}`}><div className="b225-offer-card__image"><img src={item.image} alt={item.title} loading="lazy" decoding="async"/>{item.badge?<span className="b225-offer-card__badge">{item.badge}</span>:null}<button type="button" className="b225-offer-card__heart" data-active={favorite?'true':'false'} aria-pressed={favorite} aria-label={`${favorite?'Retirer':'Ajouter'} ${item.title} ${favorite?'des':'aux'} favoris`} onClick={(event)=>{event.stopPropagation();toggleFavorite(item.id)}}>{favorite?'♥':'♡'}</button></div><div className="b225-offer-card__body"><h3 className="b225-offer-card__title">{item.title}</h3><p className="b225-offer-card__meta"><strong>{item.priceTotal}</strong><span className="b225-offer-card__dot">·</span><span>★ {item.rating}</span></p></div></article>}
function CategorySelection({id,title,items,favoriteIdSet,toggleFavorite}){
 ...
 return <section className="b225-section b225-category-selection" data-category-selection={id} data-testid={`home-selection-${id}`}><div className="b225-section__title b225-category-selection__title"><h2>{title}</h2>{items.length>1?<button type="button" className="b225-section-scroll-hint" onClick={scrollForward} aria-label={`Faire défiler ${title}`}><ScrollArrowIcon/></button>:null}</div>{items.length?<div ref={railRef} className="b225-offer-scroll">{items.map(item=><ListingCard key={`${id}-${item.id}`} item={item} sectionId={id} favorite={favoriteIdSet.has(item.id)} toggleFavorite={toggleFavorite}/>)}</div>:<p className="b225-empty">Sélection à venir.</p>}</section>
}
...
export function HomePage({onNavigate}){
 ...
 {categorySelections.map(selection=><CategorySelection key={selection.id} id={selection.id} title={selection.title} items={selection.items} favoriteIdSet={favoriteIdSet} toggleFavorite={toggleFavorite}/>)}
 </div>
}