...
export function HomePage({onNavigate}){
 ...
 <div className="b225-categories-shell"><div className="b225-categories" data-testid="home-categories">{homeCategories.map(item=><button key={item.id} type="button" data-category-id={item.id} data-active={category===item.id?'true':'false'} aria-pressed={category===item.id} onClick={()=>selectCategory(item)}><CategoryArtwork id={item.id}/>{item.label}</button>)}</div><span className="b225-category-swipe-hint" aria-hidden="true"><span>›</span><span>›</span></span></div>
