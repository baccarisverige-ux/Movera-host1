...
export function HomePage({onNavigate}){
 ...
 <div className="b225-categories-shell" aria-label="Catégories"><div ref={categoryRailRef} className="b225-categories" data-testid="home-categories"><div className="b225-categories-track">{homeCategories.map(item=><Fragment key={item.id}><button type="button" data-category-id={item.id} data-active={category===item.id?'true':'false'} aria-pressed={category===item.id} onClick={()=>selectCategory(item)}><CategoryArtwork id={item.id}/>{item.label}{item.id==='partner'?<span className="b225-category-swipe-hint" aria-hidden="true"><span>›</span><span>›</span></span>:null}</button></Fragment>)}</div></div></div>
