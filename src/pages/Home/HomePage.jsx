...
const ALL_CATEGORY_GLOBE = '__DATA__'
  ...
  return (
    <article className={`b225-card${featured ? ' b225-featured-card' : ''}`} data-testid={`home-card-${item.id}`} onClick={() => onOpen(item.id)}>
      ...
      <div className="b225-card__body">
        {featured ? <p className="b225-featured-line"><strong>{item.title}</strong><span> — {item.price} {item.currency}</span></p> : <><div className="b225-card__topline"><h3>{item.title}</h3><span>★ {item.rating || '4.90'}</span></div><p>{item.location} · Tunisie</p><p className="b225-card__price"><strong>{item.price} {item.currency}</strong> <span>/ nuit</span></p></>}
      </div>
    </article>
  )
}
...
function Collection({ title, items, onOpen }) {
  return <section className="b225-section"><div className="b225-section__title"><h2>{title}</h2><button type="button" aria-label={`Voir ${title}`}><ArrowIcon /></button></div><div className="b225-scroll">{items.map((item) => <ListingCard key={`${title}-${item.id}`} item={item} onOpen={onOpen} />)}</div></section>
}

export function HomePage({ onNavigate }) {
  const [category, setCategory] = useState('all')
  const [query, setQuery] = useState('')
  const featured = useMemo(() => { const normalizedQuery=query.trim().toLowerCase(); return homeFeatured.filter((item)=>{const categoryMatch=category==='all'||item.category.split(' ').includes(category);const queryMatch=!normalizedQuery||`${item.title} ${item.location}`.toLowerCase().includes(normalizedQuery);return categoryMatch&&queryMatch}) }, [category,query])
  const openListing=(id)=>onNavigate(`/listing/${id}`)
  return <div className="b225-home" data-testid="page-home">
    <header className="b225-home-header"><div className="b225-brand">Movera Host</div><label className="b225-search" aria-label="Rechercher une destination"><SearchIcon/><span className="b225-search__copy"><strong>Où allez-vous ?</strong><span>Destination · Dates · Voyageurs</span></span><input data-testid="home-search" type="search" value={query} onChange={(event)=>setQuery(event.target.value)} aria-label="Destination"/><button type="button" className="b225-ai" aria-label="Assistant IA" onClick={()=>onNavigate('/messages')}>✦</button></label></header>
    <div className="b225-categories" data-testid="home-categories">{homeCategories.map((item)=><button key={item.id} type="button" data-active={category===item.id?'true':'false'} onClick={()=>setCategory(item.id)}>{item.id==='all'?<img src={ALL_CATEGORY_GLOBE} alt="" aria-hidden="true" style={{width:24,height:24,objectFit:'contain',display:'block',mixBlendMode:'multiply'}}/>:<span aria-hidden="true">{item.icon}</span>}{item.label}</button>)}</div>
